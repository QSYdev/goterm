...
func (t *T) Send(stepID uint32, nc executor.NodeConfig) {
	...
	t.server.Send(qsy.NewPacket(qsy.ToucheT, uint16(nc.GetId()),
		parseColor(nc.GetColor()), nc.GetDelay(), uint16(stepID), false, false))
}
